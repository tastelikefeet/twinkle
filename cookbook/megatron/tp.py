...
def eval(model):
    ...
    dataset.set_template('Template', model_id='ms://Qwen/Qwen3-0.6B')
...
def train():
    ...
    dataloader = DataLoader(dataset=dataset, batch_size=16)
    # Use a MegatronModel
    model = MegatronModel(model_id='ms://Qwen/Qwen3-0.6B')
    ...
    model.set_lr_scheduler(scheduler_cls='default', lr_warmup_steps=0, lr_decay_steps=len(dataloader))
